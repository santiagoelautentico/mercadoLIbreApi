  ...
  class extends HTMLElement {
    ...
    listShopCard(list) {
      ...
      for (let item of list.results) {
        ...
        let secureThumbnailUrl = thumbnail.replace("http://", "https://");
        itemHtml +=
          /* html */
          `
            <item-card
...
customElements.define(
  ...
  class extends HTMLElement {
    ...
    itemDetail(item) {
      ...
      let thumbnailUrl = item.pictures[0].url;
      let secureThumbnailUrl = thumbnailUrl.replace("http://", "https://");
      let itemHtml = "";
      itemHtml += /* html */ `
        <ion-card id="${item.id}">
          <img src="${secureThumbnailUrl}" alt="" />
